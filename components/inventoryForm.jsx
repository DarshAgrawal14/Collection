...
photo: []
  ...
  useEffect(() => {
    if (id) {
      // Editing mode
      axios.get(`http://localhost:5000/api/collection/${id}`)
        .then((res) => {
          populateFormForUpdate(res.data);
          window.scrollTo(0, 0);
        })
        .catch((err) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl p-8 mb-8">
          <SectionHeader 
              icon={isUpdateMode ? "✏️" : "➕"} 
              title={isUpdateMode ? "Update Item" : "Add New Item"} 
              description={isUpdateMode 
                ? "Modify the item details and save changes." 
